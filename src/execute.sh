...
uv run python run_pipeline.py --output ../outputs/ --max-trials -1 --max-candidates 999999 --sample-seed 42 --drugbank-csv ../drugbank_approvals_smiles.csv --all-modalities --use-ct-cache --chembl-snapshot ../chembl_targets.sqlite  --opentargets-snapshot ../opentargets_snapshot.sqlite --year-range 2009-2026 --no-drop-uncached-candidates --use-ct-cache --require-smiles "$@"
