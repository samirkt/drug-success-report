...
- **Swappable adjudication**: Three adjudicators produce the same `OutcomeTable` shape and are selected via `PipelineConfig.adjudication_method` (CLI: `--adjudication-method`). (a) `"fda_timeline"` (default) — `stages/adjudication_fda.py` + `fda/` package. Reconstructs each drug's indication-level FDA approval timeline from openFDA (ORIG submissions + SUPPL new-indication letters), LLM-matches the trial indication, and checks NDC commercial status. LLM extract/match calls go through a swappable LLM backend (`AnthropicJSONClient` or `OpenAICompatJSONClient`), which shares the same `KnowledgeCache` (table: `llm_json_cache`) and `CostLedger` as the rest of the pipeline. FDA HTTP responses are cached on disk under `fda_cache_dir`. (b) `"llm_direct"` — the original `OutcomeAdjudicationStage` which asks the LLM directly per candidate, backed by the deterministic-failure shortcut and the adjudication_cache table. (c) `"ndc_indication"` — `stages/adjudication_ndc.py` + `pipeline/ndc.py`. Local-SQLite-only (no network): for each candidate, builds a synonym list (DrugBank synonyms + drug_name + drug_name_raw + mesh_drug), looks up FDA label indications via `utils/ndc_lookup.get_drugs_with_indications_bulk` (one batched call upfront for all unique drugs), then asks the LLM whether the trial indication is covered. Reuses the same `--fda-llm-*` CLI flags and the same swappable LLM backends as `fda_timeline`. Persists per-candidate verdicts to a dedicated `ndc_adjudication_cache` table for side-by-side comparison with the other methods. The no-approval → enum mapping (`ONGOING` vs `FAILED_PHASE_N`) is shared with `fda_timeline` via `pipeline/stages/_no_approval.classify_no_approval`. Module-level `_INCLUDE_COMMERCIALIZED = False` toggle in `adjudication_ndc.py` (not exposed to CLI) enables the optional `ndc_lookup.get_drug()` upgrade to `COMMERCIALIZED`. Standalone-importable: `from pipeline.ndc import NDCAdjudicator`. Requires the local FDA SQLite mirror to be built first via `python -c 'from utils.ndc_lookup import build_db; build_db()'`.
- **Swappable FDA-adjudication LLM backend**: The FDA-timeline adjudicator's LLM is itself swappable via `--fda-llm-backend {openai_compat,anthropic}` (default `openai_compat`). The default points at local Ollama (`http://localhost:11434/v1`) serving `qwen2.5:14b-instruct` — sized for a 36GB unified-memory Mac with headroom for parallel KV-cache slots; install once with `ollama pull qwen2.5:14b-instruct`. The pre-LLM `pipeline/fda/section_extract.py` narrows label/letter inputs to ~1.5-3k chars before they hit the model, which restores most of the quality lost vs the larger 32B variant. Pass `--fda-llm-model qwen2.5:32b-instruct` to opt back to the larger model, or `--fda-llm-base-url` / `--fda-llm-model` to target any OpenAI-compatible endpoint (vLLM, llama.cpp, Together, Groq, etc.). Pass `--fda-llm-backend anthropic` for Sonnet on this stage. Scope: this flag affects **FDA-timeline adjudication only** — `llm_direct` adjudication and classification continue to use Anthropic unchanged. Local models record token volume at $0 in the cost ledger (unknown model strings silently price at zero in `CostLedger`).
- **FDA-adjudication cache is rewritten unconditionally per candidate**: `FDAAdjudicator._cache_outcome` (`stages/adjudication_fda.py`) calls `put_fda_outcome` after every adjudication with no per-candidate "is this already cached?" pre-check, and `put_fda_outcome` uses `INSERT OR REPLACE`. Net effect: `src/knowledge_cache.db` is touched on every run — every cached row gets a fresh `created_at` even when nothing semantically changed. This is why git shows `knowledge_cache.db` as modified after every `execute.sh` even with `--drop-uncached-candidates` and 100% adjudication cache hits. The same pattern applies to `adjudication_ndc.py::_cache_outcome` (calls `put_ndc_outcome`). Classification and the FDA sub-LLM-call layer (`llm_json_cache`) write only on miss, so those tables don't grow on cached-only re-runs.
- **FDA-adjudication parallelism + timeouts**: `--fda-adjudication-workers N` (default 1) wraps the per-candidate loop in a `ThreadPoolExecutor`; per-drug timelines are deduplicated under a per-drug lock so candidates sharing a drug don't redundantly hit openFDA + the LLM. Set `N <= OLLAMA_NUM_PARALLEL` (the Ollama service env var); raising past that just queues at the server. On a 36GB M3 Max with the 14B default, `OLLAMA_NUM_PARALLEL=4 ollama serve` + `--fda-adjudication-workers 4` is the sweet spot. `--fda-llm-timeout` (default 180s, generous to cover cold prompts on local models) and `--fda-http-timeout` (default 30s) are surfaced so a single hung request can't wedge the run — on timeout, the candidate is recorded as `UNKNOWN` and the loop advances. Failed (non-cached) outcomes get a clean retry on the next run since only successful LLM completions persist in `llm_json_cache`.
...
- **Two-layer sizing**: `--max-trials N` caps AACT ingestion at the SQL `LIMIT` level — fast for dev runs but distortive for analysis, since M×N expansion + clustering can leave candidates with only a partial subset of their real trials. `--max-candidates N --sample-seed S` is the analysis-safe knob: after clustering (and after `--year-range` filtering) `Pipeline._sample_candidates` draws a deterministic random sample of N candidates via a fresh `random.Random(S)` and prunes `TrialTable` to the union of those candidates' `trial_ids`, so every sampled candidate keeps its complete trial set. The two flags compose.
